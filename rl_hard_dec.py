def main():
    ...
    # Keep this script RL-first by default; shared NCO shaping should only be
    # used here when the user explicitly provides rl_* overrides in config.
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for it, batch in enumerate(train_loader):
            ...
            #     # اینجا بهتره cost رو بزرگ کنی و گرادیان policy هم از logp بیاد
            #     # ولی چون cost_detached استفاده می‌کنی، حداقل سیگنال منفی شدید بده:
            #     cost_tensor = torch.as_tensor(COST_VIOLATION_WEIGHT, device=device, dtype=torch.float32)
            #     cost_detached = cost_tensor.detach()
            #     reward = -cost_detached
            #     # و ادامه بده (یا حتی continue)
    ...
    print("Log saved to rl_training_log.json")
