        ...
        *,
        ...
        for t in range(C):
            ...
            net_mask = self._network_mask_for_component(
                t=t, N=N, assignments=assignments, bw_mat=bw_mat,
                comp_predecessors=comp_predecessors, device=device
            )
            feasible = net_mask   # ظرفیت حذف شد
            ...
            if not feasible.any():
                if allow_fallback:
                    fallback_mask = torch.ones((N,), dtype=torch.bool, device=device)
                    if fixed_row >= 0:
                        fallback_mask = torch.zeros((N,), dtype=torch.bool, device=device)
                        fallback_mask[fixed_row] = True
                    if fallback_mask.any():
                        scores_fb = node_caps_2d[:, 0].masked_fill(
                            ~fallback_mask, float("-inf")
                        )
