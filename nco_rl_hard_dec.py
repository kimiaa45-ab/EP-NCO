def main():
    ...
    # PyTorch thread settings (CPU ops like matmul, etc.)
    print("torch.get_num_threads():", torch.get_num_threads())
    print("torch.get_num_interop_threads():", torch.get_num_interop_threads())

    # If you want to see/confirm env vars that often control threading
    ...
    try:
        physical = subprocess.check_output(["bash", "-lc", "lscpu | grep 'Core(s) per socket'"]).decode().strip()
        sockets  = subprocess.check_output(["bash", "-lc", "lscpu | grep 'Socket(s)'"]).decode().strip()
        threads  = subprocess.check_output(["bash", "-lc", "lscpu | grep 'Thread(s) per core'"]).decode().strip()
        print("lscpu:", physical, "|", sockets, "|", threads)
    ...
    COST_OBJECTIVE_KWARGS = {
        ...
        "w_pr": w_pr_cfg,
        "w_sr": w_sr_cfg,
        # Keep objective on the raw response-time scale in this RL script.
        # GA is still used elsewhere (baseline / reporting), but AvgObj should
        # reflect the original weighted sum, not RT divided by GA.
        "normalize_rt": False,
    ...
    def load_ga_baselines(folder, n):
        ...
        print(
            f"Loaded {sum(b is not None for b in baselines)} GA baselines (aligned 0..{n-1})"
        )
        return baselines
    ...
    ga_mask_full = torch.tensor(
        [b is not None for b in ga_baselines], dtype=torch.bool, device=device
    )
    # ------------------ Build Models ------------------
    # ====== GNN encoders ======
    from nets.encoder.servicegnn import ServiceGNN
    from nets.encoder.nodegnn import NodeGNN
    # ServiceGNN config: برای هر service یک ماژول
    # (اگه service_id ها 0..num_services-1 هستن این درست کار می‌کنه)
    service_gnn_cfg = {
        int(sid): {
            "in_dim": 3,  # component features: [cpu, mem, reliability]
            "model_dim": hidden_dim,  # خروجی embedding = hidden_dim
            "n_layers": n_layers,  # همون n_encode_layers
            "use_log1p": True,
            "ds_mode": "inv",
        }
        for sid in range(int(num_services))
    }
    # به خاطر اینکه پایین‌تر کلی جا از اسم comp_encoder/node_encoder استفاده شده،
    # همون اسم‌ها رو نگه می‌داریم تا کمترین تغییر لازم باشه.
    comp_encoder = ServiceGNN(service_config=service_gnn_cfg, device=device).to(device)
    # NodeGNN: node_graph.x in dataset is [cpu, mem, nodeID, tier, reliability]
    # so in_node_feat=5
    node_encoder = NodeGNN(
        in_node_feat=5, in_edge_feat=2, dim=hidden_dim, n_layers=n_layers
    ).to(device)
    # ====== Decoder ======
    ...
    # pipeline = ARPlacementPipeline(embed_dim=hidden_dim, proj_dim=8, cap_dim=2).to(
    #     device
    # )
    ...
    optimizer = optim.Adam(
        [
            {"params": list(comp_encoder.parameters()) + list(node_encoder.parameters()), "lr": base_lr},
    ...
    separate_eval_decoder_shaping = bool(
        config.get("separate_eval_decoder_shaping", False)
    )
    if separate_eval_decoder_shaping:
    ...
    anneal_epochs=int(0.6 * num_epochs)
    ...
    selection_metric_cfg = str(
        model_cfg.get("selection_metric", config.get("selection_metric", "auto"))
    ).strip().lower()
    if selection_metric_cfg == "auto":
        selection_metric = (
            "response_time"
            if abs(w_pr_cfg) < 1e-12 and abs(w_sr_cfg) < 1e-12
            else "objective"
        )
    elif selection_metric_cfg in ("objective", "response_time"):
        selection_metric = selection_metric_cfg
    else:
        selection_metric = "objective"
    min_delta_rt = float(config.get("min_delta_rt", min_delta))
    epochs_no_improve = 0
    ...
    print(
        f"Model selection metric: {selection_metric} "
        f"(min_delta_metric={min_delta_rt if selection_metric == 'response_time' else min_delta})"
    )
    print(
    ...
    print(f"Seed: {SEED}")
    print(f"Decoder shaping (train): {DECODER_SHAPING}")
    print(f"Decoder shaping (eval):  {EVAL_DECODER_SHAPING}")
    ...
    platform_reliability_history = []
    service_reliability_history = []
    min_cost_history = []
    max_cost_history = []
    train_eval_history = []
    greedy_avg_history = []
    greedy_min_history = []
    greedy_max_history = []
    eval_objective_history = []
    eval_response_time_history = []
    eval_platform_reliability_history = []
    eval_service_reliability_history = []
    improved_epochs = []
    improved_costs = []
    prev_val_assign_signatures = None
    ...
    policy_batch_size = int(config.get("policy_batch_size", 16))
    ...
    policy_cost_mode_cfg = str(config.get("policy_cost_mode", "auto")).strip().lower()
    if policy_cost_mode_cfg == "auto":
        policy_cost_mode = (
            "response_time"
            if selection_metric == "response_time"
            and abs(w_pr_cfg) < 1e-12
            and abs(w_sr_cfg) < 1e-12
            else "objective"
        )
    else:
        policy_cost_mode = policy_cost_mode_cfg
    allow_fallback_train = bool(config.get("allow_fallback_train", True))
    allow_fallback_eval = bool(config.get("allow_fallback_eval", True))
    if device.type == "cpu":
        cpu_policy_batch_size = int(config.get("cpu_policy_batch_size", 4))
        if policy_batch_size > cpu_policy_batch_size:
            print(
                f"CPU mode: reducing policy_batch_size from {policy_batch_size} "
                f"to {cpu_policy_batch_size} to avoid memory growth."
            )
            policy_batch_size = cpu_policy_batch_size
    print(
        f"Policy baseline settings: use_ga_baseline_for_policy={use_ga_baseline_for_policy}, "
        f"normalize_advantage={normalize_advantage}, policy_batch_size={policy_batch_size}"
    )
    ...
    # ------------------ Greedy eval helper ------------------
    # @torch.no_grad()
    # def greedy_eval_subset(max_samples=None):
    #     comp_encoder.eval()
    #     node_encoder.eval()
    #     pipeline.eval()
    #     costs = []
    ...
    #         sample = batch[0]
    #         # NOTE: Keep python lists (e.g., comp_predecessors) as-is
    #         sample = {
    #             k: (
    #                 to_device(v, device)
    ...
    #         # comp_emb = comp_encoder(sb_batch.x[:, :2])
    #         # node_emb = node_encoder(sample["node_graph"].x)
    #         # ServiceGNN expects the Batch itself (must carry service_id)
    #         comp_emb, _ = comp_encoder(sb_batch)
    #         # NodeGNN expects the PyG Data object
    #         node_emb = node_encoder(sample["node_graph"])
    #         # -------- decoder inputs (2-resource) --------
    #         comp_reqs = to_device(
    #             sample["comp_reqs"], device
    #         )  # [C] CPU-only in dataset? -> we fix below
    #         node_caps_cpu = to_device(
    #             sample["node_caps_cpu"], device
    #         )  # [N] CPU-only in dataset
    #         # Your model & costfunction uses CPU+MEM; the easiest consistent path:
    #         # derive 2D req/caps from graphs (authoritative source)
    #         comp_reqs2 = sb_batch.x[:, :2].to(torch.float32)  # [C,2] cpu,mem
    #         node_caps2 = (
    #             sample["node_graph"].x[:, :2].to(torch.float32)
    #         )  # [N,2] cpu,mem
    ...
    #             # برای eval: مستقیم یک هزینه خیلی بزرگ بده تا میانگین خراب بشه و بفهمی مشکل داری
    #             costs.append(float(COST_VIOLATION_WEIGHT))
    #             continue
    #         res = compute_cost(
    #             assignments,
    #             sample["node_graph"],
    ...
    #             #lambda_u = 0.5,
    #             lambda_u=0.5,              # شروع پیشنهادی: 0.01 تا 0.05
    #             lambda_spread=0.2,          # شروع پیشنهادی: 0.2 تا 2.0
    #             min_unique_nodes=2,         # حداقل 2 نود برای هر سرویس
    #             spread_guard="always",            
    #         )
    #         total = res["total_response_time"]
    #         costs.append(float(total.detach().cpu().item()))
    #         seen += 1
    #         if max_samples is not None and seen >= max_samples:
    #             break
    #     if len(costs) == 0:
    #         return None, None, None
    #     return sum(costs) / len(costs), min(costs), max(costs)
    # # Epoch 0 eval
    # avg0, min0, max0 = greedy_eval_subset(MAX_EVAL_SAMPLES)
        
    # import random

    # @torch.no_grad()
    # def random_eval_subset(max_samples=None, seed=0):
    #     # برای اینکه eval اول هر دفعه یکسان باشه (اختیاری)
    #     rnd = random.Random(seed)
    ...
    #         C = int(sb_batch.x.size(0))
    #         N = int(node_graph.x.size(0))

    #         bw_mat = sample["bw_mat"]
    #         if isinstance(bw_mat, torch.Tensor) and bw_mat.dim() == 3:
    #             bw_mat = bw_mat[0]
    #         bw_mat = bw_mat.to(device=device)

    #         comp_predecessors = sample["comp_predecessors"]  # List[List[int]]

    #         # ---- random assignments but only among feasible nodes (network-feasible) ----
    #         assignments = torch.full((C,), -1, dtype=torch.long, device=device)

    #         for t in range(C):
    #             preds = comp_predecessors[t]
    #             if not preds:
    #                 feasible = torch.ones((N,), dtype=torch.bool, device=device)
    #             else:
    #                 feasible = torch.ones((N,), dtype=torch.bool, device=device)
    ...
    #             feas_idx = feasible.nonzero(as_tuple=False).view(-1)
    #             # انتخاب تصادفی یکنواخت از بین feasible ها
    #             j = feas_idx[rnd.randrange(int(feas_idx.numel()))]
    #             assignments[t] = j
    ...
    # # Epoch 0 eval
    # #avg0, min0, max0 = greedy_eval_subset(MAX_EVAL_SAMPLES)
    # avg0, min0, max0 = random_eval_subset(MAX_EVAL_SAMPLES)
    ...
    # if avg0 is not None:
    #     greedy_avg_history.append(avg0)
    #     greedy_min_history.append(min0)
    #     greedy_max_history.append(max0)
    #     train_eval_history.append(avg0)
    #     # best_eval_cost = avg0
    #     # best_eval_epoch = 0
    #     best_val_cost = avg0
    #     best_val_epoch = 0
    #     if VERBOSE:
    #         print(
    #             f"Epoch 0 Greedy Eval: avg={avg0:.6f}, min={min0:.6f}, max={max0:.6f}"
    #         )
    ema_baseline = None
    ema_beta = 0.98
    def get_lambda_u(epoch: int) -> float:
        progress = min(1.0, epoch / float(anneal_epochs))
        return float(0.3 + (0.15 - 0.3) * progress)  # linear decay
    ...
    def get_policy_cost_from_result(res):
        ...
        if policy_cost_mode in ("response_time", "rt"):
            # Train on raw RT plus only hard placement violations.
            # Soft regularizers like util/spread/GA-guard were dominating the
            # gradient and keeping greedy eval unchanged.
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        entropy_coeff = final_entropy + (init_entropy - final_entropy) * np.exp(
            -progress * 3
        )
        ...
        epoch_policy_t = torch.zeros((), device=device)
        epoch_reward_t = torch.zeros((), device=device)
        epoch_rt_t = torch.zeros((), device=device)
        epoch_pr_t = torch.zeros((), device=device)
        epoch_sr_t = torch.zeros((), device=device)
        epoch_viol_t = torch.zeros((), device=device)
        epoch_entropy_t = torch.zeros((), device=device)
        epoch_feasible_t = torch.zeros((), device=device)
        epoch_feasible_cnt = 0
        batch_costs_cpu = []
        batch_rt_cpu = []
        batch_pr_cpu = []
        batch_sr_cpu = []
        ...
        for it, batch in enumerate(train_loader):
            ...
            try:
                ...
                if ("out of memory" in msg) or ("not enough memory" in msg):
                    print(
                        f"[WARN] Memory pressure at epoch={epoch}, iter={it}; "
                        f"clearing grads/buffers and skipping this sample."
                    )
                    optimizer.zero_grad(set_to_none=True)
                    mb_logps.clear()
                    mb_entropies.clear()
            ...
            assignments = out["assignments"]
            if "entropies" in out and isinstance(out["entropies"], torch.Tensor):
                epoch_entropy_t = epoch_entropy_t + out["entropies"].detach().mean()
            if "feasible_counts" in out and isinstance(out["feasible_counts"], torch.Tensor):
                epoch_feasible_t = epoch_feasible_t + out["feasible_counts"].detach().sum()
                epoch_feasible_cnt += int(out["feasible_counts"].numel())
            if (assignments < 0).any():
                # Keep policy-gradient terms, but score invalid assignment with real cost.
                logp = out["log_probs"].sum()
                # Entropy should be normalized per decision; summing across all
                # components was overpowering the policy gradient and keeping the
                # decoder near-random.
                ...
                bad_obj = bad_res["objective"]
                if not isinstance(bad_obj, torch.Tensor):
                    bad_obj = torch.as_tensor(bad_obj, device=device, dtype=torch.float32)
                ...
                batch_costs_cpu.append(float(cost_detached.detach().cpu().item()))
                bad_rt = bad_res["response_time"]
                bad_pr = bad_res["platform_reliability"]
                bad_sr = bad_res["service_reliability"]
                if not isinstance(bad_rt, torch.Tensor):
                    bad_rt = torch.as_tensor(bad_rt, device=device, dtype=torch.float32)
                if not isinstance(bad_pr, torch.Tensor):
                    bad_pr = torch.as_tensor(bad_pr, device=device, dtype=torch.float32)
                if not isinstance(bad_sr, torch.Tensor):
                    bad_sr = torch.as_tensor(bad_sr, device=device, dtype=torch.float32)
                epoch_cost_t = epoch_cost_t + bad_obj.detach()
                epoch_policy_t = epoch_policy_t + cost_detached
                epoch_reward_t = epoch_reward_t - cost_detached
                epoch_rt_t = epoch_rt_t + bad_rt.detach()
                epoch_pr_t = epoch_pr_t + bad_pr.detach()
                epoch_sr_t = epoch_sr_t + bad_sr.detach()
                epoch_viol_t = epoch_viol_t + _ensure_scalar_tensor(
                    bad_res.get("violations", 0.0)
                ).detach()
                batch_rt_cpu.append(float(bad_rt.detach().cpu().item()))
                batch_pr_cpu.append(float(bad_pr.detach().cpu().item()))
                batch_sr_cpu.append(float(bad_sr.detach().cpu().item()))
                # Keep training signal even on invalid assignments.
                # Otherwise, epochs with all-invalid samples produce zero optimizer steps.
                if (len(mb_sample_idx) == policy_batch_size) or is_last:
                    ...
                    mb_idx_tensor = torch.tensor(
                        mb_sample_idx, dtype=torch.long, device=device
                    )
                    mb_idx_tensor = torch.clamp(
                        mb_idx_tensor, 0, ga_tensor_full.numel() - 1
                    )
                    ...
                    epoch_updates += 1
                    # Update EMA after policy update to avoid leaking current batch into baseline.
            ...
            ga_ref_val = None
            if 0 <= sidx_for_ref < ga_tensor_full.numel() and bool(ga_mask_full[sidx_for_ref].item()):
            ...
            rt_tensor = res["response_time"]
            if not isinstance(rt_tensor, torch.Tensor):
                rt_tensor = torch.as_tensor(
                    rt_tensor, device=device, dtype=torch.float32
                )
            pr_tensor = res["platform_reliability"]
            if not isinstance(pr_tensor, torch.Tensor):
                pr_tensor = torch.as_tensor(
                    pr_tensor, device=device, dtype=torch.float32
                )
            sr_tensor = res["service_reliability"]
            if not isinstance(sr_tensor, torch.Tensor):
                sr_tensor = torch.as_tensor(
                    sr_tensor, device=device, dtype=torch.float32
                )
            ...
            entropy = out["entropies"].mean()  # normalized entropy regularizer
            mb_logps.append(logp)
            mb_entropies.append(entropy)
            ...
            epoch_policy_t = epoch_policy_t + policy_cost_tensor.detach()
            epoch_reward_t = epoch_reward_t + reward.detach()
            epoch_rt_t = epoch_rt_t + rt_tensor.detach()
            epoch_pr_t = epoch_pr_t + pr_tensor.detach()
            epoch_sr_t = epoch_sr_t + sr_tensor.detach()
            epoch_viol_t = epoch_viol_t + _ensure_scalar_tensor(
                res.get("violations", 0.0)
            ).detach()
            batch_costs_cpu.append(float(cost_tensor.detach().cpu().item()))
            batch_rt_cpu.append(float(rt_tensor.detach().cpu().item()))
            batch_pr_cpu.append(float(pr_tensor.detach().cpu().item()))
            batch_sr_cpu.append(float(sr_tensor.detach().cpu().item()))
            # ---------- mini-batch update ----------
        ...
        avg_policy_cost = float(
            (epoch_policy_t / max(1, num_train)).detach().cpu().item()
        )
        avg_reward = float((epoch_reward_t / max(1, num_train)).detach().cpu().item())
        avg_rt = float((epoch_rt_t / max(1, num_train)).detach().cpu().item())
        avg_pr = float((epoch_pr_t / max(1, num_train)).detach().cpu().item())
        avg_sr = float((epoch_sr_t / max(1, num_train)).detach().cpu().item())
        avg_viol = float((epoch_viol_t / max(1, num_train)).detach().cpu().item())
        avg_entropy = float(
            (epoch_entropy_t / max(1, num_train)).detach().cpu().item()
        )
        avg_feasible = (
            float((epoch_feasible_t / max(1, epoch_feasible_cnt)).detach().cpu().item())
            if epoch_feasible_cnt > 0
            else float("nan")
        )
        ...
        objective_history.append(avg_cost)
        response_time_history.append(avg_rt)
        platform_reliability_history.append(avg_pr)
        service_reliability_history.append(avg_sr)
        ...
        if VERBOSE:
            ...
            print(
                ...
                f"AvgObj={avg_cost:.6f} AvgPolicy={avg_policy_cost:.6f} AvgRT={avg_rt:.6f} "
                f"AvgPR={avg_pr:.6f} AvgSR={avg_sr:.6f} AvgReward={avg_reward:.6f} "
                f"AvgViol={avg_viol:.6f} AvgEntropy={avg_entropy:.6f} AvgFeasible={avg_feasible:.2f} "
                f"BaselineMean={last_baseline_mean:.6f} Temp={temperature:.4f} lambda_u={lambda_u:.3f}"
            )
            ...
            with torch.no_grad():
                ...
                for batch in val_loader:
                    ...
                    node_state = NodeState(
                        node_caps=node_caps2, device=device, warn_once=False
                    )
                    bw_mat = sample["bw_mat"]
                    if isinstance(bw_mat, torch.Tensor) and bw_mat.dim() == 3:
                        bw_mat = bw_mat[0]
                    ...
                    assignments = out["assignments"]
                    eval_assign_signatures.append(
                        (
                            int(sample.get("sample_idx", -1)),
                            tuple(assignments.detach().cpu().tolist()),
                        )
                    )
                    if (assignments < 0).any():
                        sidx_for_ref = int(sample.get("sample_idx", 0))
                        ...
                        **COST_OBJECTIVE_KWARGS,
                        ...
                        bad_obj = bad_eval["objective"]
                        bad_rt = bad_eval["response_time"]
                        bad_pr = bad_eval["platform_reliability"]
                        bad_sr = bad_eval["service_reliability"]
                        eval_costs.append(
                            float(bad_obj.detach().cpu().item())
                            if isinstance(bad_obj, torch.Tensor)
                            else float(bad_obj)
                        )
                        eval_rts.append(
                            float(bad_rt.detach().cpu().item())
                            if isinstance(bad_rt, torch.Tensor)
                            else float(bad_rt)
                        )
                        eval_prs.append(
                            float(bad_pr.detach().cpu().item())
                            if isinstance(bad_pr, torch.Tensor)
                            else float(bad_pr)
                        )
                        eval_srs.append(
                            float(bad_sr.detach().cpu().item())
                            if isinstance(bad_sr, torch.Tensor)
                            else float(bad_sr)
                        )
                        seen += 1
                    ...
                    cost_eval = compute_cost(
                        ...
                        lambda_spread=eval_lambda_spread,
                        min_unique_nodes=2,         # حداقل 2 نود برای هر سرویس
                        spread_guard="always",      # نیازمند Patch
                        ga_ref=ga_ref_val,
                comp_fixed_node_rows=sample.get("comp_fixed_node_rows", None),
                **COST_OBJECTIVE_KWARGS,
                    )
                    ...
                    if not isinstance(total, torch.Tensor):
                        total = torch.as_tensor(
                            total, device=device, dtype=torch.float32
                        )
                    eval_costs.append(float(total.detach().cpu().item()))
                    rt_eval = cost_eval["response_time"]
                    pr_eval = cost_eval["platform_reliability"]
                    sr_eval = cost_eval["service_reliability"]
                    eval_rts.append(float(rt_eval.detach().cpu().item()) if isinstance(rt_eval, torch.Tensor) else float(rt_eval))
                    eval_prs.append(float(pr_eval.detach().cpu().item()) if isinstance(pr_eval, torch.Tensor) else float(pr_eval))
                    eval_srs.append(float(sr_eval.detach().cpu().item()) if isinstance(sr_eval, torch.Tensor) else float(sr_eval))
                    seen += 1
                    if MAX_EVAL_SAMPLES is not None and seen >= MAX_EVAL_SAMPLES:
                        break
            if len(eval_costs) > 0:
                ...
                avg_val_rt = (
                    float(sum(eval_rts) / len(eval_rts)) if len(eval_rts) > 0 else 0.0
                )
                ...
                eval_objective_history.append(avg_val_eval)
                eval_response_time_history.append(avg_val_rt)
                eval_platform_reliability_history.append(float(sum(eval_prs) / len(eval_prs)) if len(eval_prs) > 0 else 0.0)
                eval_service_reliability_history.append(float(sum(eval_srs) / len(eval_srs)) if len(eval_srs) > 0 else 0.0)
                ...
                if VERBOSE:
                    print(
                        f"Greedy VAL eval | Obj(avg/min/max)=({avg_val_eval:.6f}/"
                        f"{min_val_eval:.6f}/{max_val_eval:.6f}) | RT(avg)={avg_val_rt:.6f}"
                    )
                    if prev_val_assign_signatures is not None:
                        total_cmp = min(
                            len(prev_val_assign_signatures),
                            len(eval_assign_signatures),
                        )
                        changed = 0
                        for i in range(total_cmp):
                            if prev_val_assign_signatures[i] != eval_assign_signatures[i]:
                                changed += 1
                        print(
                            f"Greedy VAL assignment changes vs last eval: {changed}/{total_cmp}"
                        )
                prev_val_assign_signatures = eval_assign_signatures
                # improved = avg_train_eval < best_eval_cost - min_delta
                monitored_val = (
                    avg_val_rt if selection_metric == "response_time" else avg_val_eval
                )
                metric_delta = (
                    min_delta_rt if selection_metric == "response_time" else min_delta
                )
                improved = monitored_val < best_val_cost - metric_delta
                if improved:
                    ...
                    ckpt = {
                        ...
                        "best_val_objective": float(avg_val_eval),
                        "best_val_response_time": float(avg_val_rt),
                    ...
                    print(
                        ...
                        f"{selection_metric}={best_val_cost:.6f} "
                        f"(obj={avg_val_eval:.6f}, rt={avg_val_rt:.6f})"
                    )
                else:
                    epochs_no_improve += 1
                    # if avg_train_eval > last_eval_cost:
                    if monitored_val > last_val_cost:
                    ...
                    if epoch >= min_epochs:
                        if consecutive_increases >= max_consecutive_increases:
                            print(
                                f"Stopping: {max_consecutive_increases} consecutive increases"
                            )
    ...
    print(
        f"Training finished at {training_end_dt} (UTC). Total duration: {total_training_seconds}s"
    )
    log_data = {
        ...
        "platform_reliability_history": platform_reliability_history,
        "service_reliability_history": service_reliability_history,
        ...
        "eval_objective_history": eval_objective_history,
        "eval_response_time_history": eval_response_time_history,
        "eval_platform_reliability_history": eval_platform_reliability_history,
        "eval_service_reliability_history": eval_service_reliability_history,
        "final_epoch": epoch,
    ...
    log_path = "nco_hard_training_log.json"
