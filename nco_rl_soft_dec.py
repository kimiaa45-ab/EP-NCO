def main():
    ...
    COST_OBJECTIVE_KWARGS = {
        ...
        "w_pr": w_pr_cfg,
        "w_sr": w_sr_cfg,
        # Keep RL monitoring on the raw response-time scale.
        "normalize_rt": False,
    ...
    # ------------------ Checkpoint load ------------------
    ckpt_path = "best_nco_soft_model.pt"
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for it, batch in enumerate(train_loader):
            ...
            if (assignments < 0).any():
                # logp/entropy رو نگه دار تا گرادیان بیاد
            ...
            if (len(mb_sample_idx) == policy_batch_size) or is_last:
                ...
                fallback_val = ema_baseline
                baseline = torch.where(mask_batch, ga_batch, fallback_val)  # [B]
                if not use_ga_baseline_for_policy:
        ...
        if VERBOSE:
            ...
            print(
                ...
                f"AvgViol={avg_viol:.6f} AvgEntropy={avg_entropy:.6f} "
                f"AvgFeasible={avg_feasible:.3f} BaselineMean={last_baseline_mean:.6f} "
                f"Temp={temperature:.4f} lambda_u={lambda_u:.3f}"
            )
        ...
        if epoch % EVAL_EVERY == 0:
            ...
            with torch.no_grad():
                ...
                for batch in val_loader:
                    ...
                    if abs(eval_lambda_spread - 0.2) > 1e-12:
                        cost_eval = compute_cost(
                            assignments,
                            sample["node_graph"],
                            sample["service_batch"],
    ...
    log_path = "nco_soft_training_log.json"
